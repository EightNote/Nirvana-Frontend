...
const Search = () => {
  ...
  let results_available = (matchedAlbums, matchedArtists, matchedTracks) => [
  ...
  useEffect(() => {
    ...
    let base_search_url = "http://localhost:8080/search?q="
    let search_string_url = searchString.replace(' ', '<space>')

    axios
      .get(base_search_url + search_string_url, {
        headers: {
          Authorization: "Bearer " + user, //the token is a variable which holds the token
        },
      })
      .then((response) => {
        setMatchedAlbums(response.data);
      });
    ...
    axios
      ...
      .then((response) => {
        setMatchedTracks(response.data);
      });

    setSearchResults(results_available(matchedAlbums, matchedArtists, matchedTracks))
  }, [])

   
  return (
    <Stack display="flex" justifyContent={"center"} alignItems={"stretch"} direction="column" spacing = {10} >
      <hr />
        <Box bgcolor={"white"} borderRadius={"20px"} padding={"10px 40px"} border={"2px solid white"} margin={"50px 100px"}  display="flex" justifyContent={"center"} >
          <SearchBar ref = {searchTextFieldRef} onChange={onChange}/>
        </Box>
      <hr />
        {searchResults}
      <hr />
